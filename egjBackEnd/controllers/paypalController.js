...
const getCaptureDetails = ({ capture, captureOrder, booking }) => {
  ...
  const fee = selectedCapture.seller_receivable_breakdown?.paypal_fee?.value
    ? parseFloat(selectedCapture.seller_receivable_breakdown.paypal_fee.value)
    : 0;
  const netAmount = selectedCapture.seller_receivable_breakdown?.net_amount?.value
    ? parseFloat(selectedCapture.seller_receivable_breakdown.net_amount.value)
    : parseFloat(selectedCapture.amount?.value ?? 0) - fee;
...
const finalizePaidBooking = async ({ booking, capture, captureOrder }) => {
  ...
  const updatedBooking = await Booking.findByIdAndUpdate(
    booking._id,
    {
      ...
      "paypal.fee": captureDetails.fee,
      "paypal.netAmount": captureDetails.netAmount,
